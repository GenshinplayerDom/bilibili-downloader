...
class Downloader {
  constructor(options = {}) {
    ...
    this.idleTimeout = options.idleTimeout || 20000;   // 传输中途无数据超过该时长视为停滞，中止并重试
  ...
  async transfer(url, dest, range, total, signal, progress) {
    ...
    try {
      ...
      let received = 0;
      // idle 停滞检测：CDN 传输中途若长时间无数据（TCP 心跳会让 socket 超时失效），
      // 主动中止该分片并触发外层重试，避免任务卡死在某一进度。
      const iterator = res[Symbol.asyncIterator]();
      let lastData = Date.now();
      const idleTimer = setInterval(() => {
        if (Date.now() - lastData > this.idleTimeout) res.destroy(new Error('下载停滞（' + Math.round(this.idleTimeout / 1000) + 's 无数据），已自动重试'));
      }, 5000);
      try {
        for (;;) {
  ...
  async download(urls, dest, options = {}) {
    ...
    // 断点续传：使用已有分片目录并跳过已完成分片；否则新建（目录名带目标文件名便于识别）
    const doneParts = new Map();
    let temp;
    if (options.resumeDir && fs.existsSync(options.resumeDir)) {
      temp = options.resumeDir;
      try {
        fs.readdirSync(temp).forEach(function (f) {
          if (/^\d+$/.test(f)) doneParts.set(Number(f), fs.statSync(path.join(temp, f)).size);
        });
      } catch (e) { }
    } else {
      const safeBase = String(path.basename(dest) || 'dl').replace(/[^\w.-]/g, '_').slice(0, 60);
      temp = fs.mkdtempSync(path.join(path.dirname(dest), '.bili-parts-' + safeBase + '-'));
    }
    if (typeof options.onTemp === 'function') { try { options.onTemp(temp); } catch (e) { } }
    const parts = [];
    const progress = new Map(doneParts);
    let keptTemp = null;
    ...
    const worker = async () => {
      while (!failed && next < parts.length) {
        const index = next++;
        if (doneParts.has(index)) {
          const expect = parts[index] ? Math.min(this.chunkSize, info.total - parts[index].start) : info.total;
          if (doneParts.get(index) === expect) { notify(false); continue; }
          try { fs.rmSync(path.join(temp, String(index)), { force: true }); } catch (e) { }
        }
        let completed = false;
    ...
    try {
      ...
      notify(true);
      return { size: fs.statSync(dest).size, tempDir: null };
    } catch (error) {
      // 取消 / 暂停且要求保留分片时，不删除分片目录（供跨重启续传）；成功路径始终清理
      if (options.keepPartsOnAbort && (control.paused || control.cancelled) && !fs.existsSync(dest)) {
        keptTemp = temp;
        if (typeof options.onTemp === 'function') { try { options.onTemp(temp); } catch (e) { } }
      }
      throw error;
    } finally {
      // 注意：不得在此 return，否则会吞掉上面的异常
      if (!keptTemp) fs.rmSync(temp, { recursive: true, force: true });
    }
  }
}
