...
- `npm test`：本地 HTTP 模拟服务验证安全边界、分片校验、暂停恢复、取消、重试和签名。
- `npm run check`：JavaScript 语法和 Android 前端资源一致性。
- `npm run smoke`：隐藏窗口的 Electron 界面测试，使用独立临时配置、模拟媒体和下载目录，不登录真实账号。
- `npm run pack:win`：生成 `dist/win-unpacked/` 可运行目录。
- `npm run dist:win`：生成 `dist/` 下的便携版和 NSIS 安装包。
